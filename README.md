...
for a fixed set of reference vectors. It targets the regime $d \gg r$, where $d$
is the vector dimension and $r$ is the number of reference vectors. Approximate
queries access only selected coordinates to return an approximate nearest
neighbor under $\ell_1$ or $\ell_2$ distance. Reference vectors can be class
centroids or individual data points. This is a research implementation for that
specialized regime, not a general replacement for large-scale ANN libraries.
...
[literature/HNSVW.pdf](literature/HNSVW.pdf) under the publication's
[CC BY 4.0 license](https://creativecommons.org/licenses/by/4.0/).

## Algorithmic overview

In the Nearest Neighbor Search Problem, we are given a set $R \subseteq \mathbb{R}^d$ of reference vectors and a query vector $q\in \mathbb{R}^d$ and the goal is to find the vector $v \in R$ that minimizes the distance between $v$ and $q$. We denote $r$ as the cardinality of $R$.

The reference vectors are known in advance and preprocessed into an index that answers subsequent queries.

Comparing every coordinate is expensive when vectors have many dimensions. The idea is to sample coordinates using information from the reference vectors and estimate distances from those samples.
...
the reference vectors. In the theoretical construction, this sampling is repeated $O(\log r)$ times. In this implementation, we expose this as a parameter $T$ determining how often this sampling is repeated. We call $I$ the union of the multisets over the $T$ iterations.


The flat index transforms reference vectors by restricting them to the coordinates in $I$ and scaling the coordinates according to their sampling probability. Whenever a query arrives, we apply the same procedure to the query and compute the distance between all of the restricted reference vectors and the query. We return the reference vector that minimizes this distance.

### Hierarchical Index

The hierarchical index projects the restricted reference vectors from the flat index further down by multiplying all of them with one random matrix of dimension $|I| \times p$. The parameter $p$ is fixed to $O(\log r)$ in the theoretical construction and remains exposed as a parameter in this implementation. Details about these matrices used for the projection can be found in the theoretical paper. The same projection is also applied to the query. We then estimate distances in the projected space and return the reference vector with the smallest estimate.
...
Agreement with exact search is the percentage of queries that return the same
reference vector as the exact baseline. Relative distance excess measures how
much farther the returned vector is from the query compared with that baseline.

### Single-threaded CPU: 33 class centroids

On the TCGA Pan-Cancer $\ell_2$ benchmark with 33 class centroids (one per cancer
project), 60,660 dimensions, and 3,013 held-out queries, flat importance sampling
gives the best observed trade-off between query latency and agreement with
exact search. Approximate results are arithmetic means over five sampling seeds.
Exact search was run once. At $T=8$, flat search agrees with exact search on
90.1% of queries, is approximately 600 times faster, and accesses 0.14% of
coordinates. At $T=32$, agreement rises to 94.5%. 0.20% of queries have a returned
distance more than 10% above the exact distance, while search remains
approximately 155 times faster. At
...
frontier at $r=33$. With only 33 vectors to compare, the reduced final comparison
...
The trade-off between query latency and agreement with exact search is shown
below. Its configurations are defined by the
[TCGA Pan-Cancer r=33 L2 CPU sparse sweep](experiments/tcga_pancancer/cpu/tcga_pancancer_r33_l2_cpu_sparse_sweep.tsv).

![TCGA Pan-Cancer r=33 L2 CPU query latency versus agreement with exact search](assets/tcga_pancancer_r33_l2_cpu_sparse_sweep_latency_agreement_pareto.png)
...
with the same expected number of coordinate samples. Uniform sampling has
...
![TCGA Pan-Cancer r=33 L2 CPU uniform vs flat query latency versus agreement with exact search](assets/tcga_pancancer_r33_l2_cpu_uniform_vs_flat_latency_agreement_pareto.png)

The underlying measurements are available in the
[machine-readable r=33 CPU uniform-versus-flat report](assets/tcga_pancancer_r33_l2_cpu_uniform_vs_flat.json).

### CUDA: 7,028 reference vectors

The intended benefit of the hierarchical index becomes more plausible as $r$ grows, but
...
training samples as reference vectors, the same 60,660 dimensions, and all 3,013
...
With direct kernels, some hierarchical configurations offer both lower query
latency and lower mean relative distance excess than flat configurations.
For example, hierarchical $(T,p)=(16,128)$ takes 147 microseconds per
...
reverses at smaller distance errors: hierarchical $(128,1536)$ takes
1.039 milliseconds per query and flat $T=48$ takes 1.018 milliseconds, while
flat also has the lower mean excess, 0.424% versus 0.536%.

The result is strategy-dependent. SGEMM makes flat scans much cheaper, and the
hierarchy does not show the same clear advantage. Flat $T=48$ takes
66.4 microseconds per query with 0.424% mean excess, while hierarchical
$(128,1536)$ takes 77.5 microseconds with 0.536%.
The direct CUDA exact scan is the FP32 reference. The unrefined SGEMM exact scan
selects a different reference vector for 7 of 3,013 queries, although its maximum
distance excess is below 0.008%.

![TCGA Pan-Cancer r=7,028 L2 CUDA query latency versus mean relative distance excess](assets/tcga_pancancer_r7028_l2_cuda_crossover_latency_distance_excess_mean.png)
...
query transfer, projection, scan of reference vectors, and result transfer as
...
The final command generates a deterministic dataset with 64 reference vectors
and 512 queries in 16,384 dimensions, runs a compact L2 sweep, and creates a
plot of query latency versus agreement with exact search. It compares exact
search with flat importance sampling, mass-matched uniform sampling, and a small
hierarchical grid. This is an illustrative smoke benchmark, not evidence for
the paper or real-data claims. Its generated data, report, and figures remain
ignored by Git.

The default build produces `nearest_neighbor_benchmark`, the L1 and L2
...
documented Quadro T1000, constructing the required `gpu_fp32` probability file
for 7,028 reference vectors takes approximately five minutes. Once the
...
The transform creates the 33 class centroids and held-out queries used
by the CPU experiments. `--write-training-pool` also materializes the
7,028 individual training vectors required by the large CUDA setup.
...
./build-cuda/compute_l2_probabilities \
    --input data/processed/tcga_pancancer_v1/training_pool.npy \
...
matrix of reference vectors. The benchmark recomputes and verifies that binding
before reuse, so a stale probability file from a different same-shaped matrix is
...
and emits a canonical schema-version 6 JSON report plus trial-level CSV. The
reporting tools reject older report layouts. The JSON provenance block binds
the result to its source commit, build and execution environment, setup,
dataset matrices, and optional probability file. Large-pool setups use `selected_distances`
diagnostics. `full_distance_table` additionally supports ranks and margins at
the cost of evaluating every pair of a query and a reference vector. The
