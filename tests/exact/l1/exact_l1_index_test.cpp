...
int main()
{
    ...
    static_assert(
        !std::is_constructible_v<ExactL1Index, DenseMatrix&&>);
    static_assert(
    ...
    const DenseMatrix representatives(
        ...
        3,
        2);
    const ExactL1Index index(representatives);
    ...
    const std::array<float, 2> near_third{
        2.0F,
        3.0F};
    passed &= expect(
        index.query(std::span<const float>{near_third}) == 2,
        "all query coordinates must contribute to the distance");

    const std::array<float, 2> tied{
        2.5F,
        0.0F};
    passed &= expect(
    ...
    try {
        static_cast<void>(
            index.query(
                std::span<const float>{wrong_dimension}));
    ...
    try {
        static_cast<void>(
            index.query(
                std::span<const float>{non_finite_query}));
    } catch (const std::invalid_argument&) {
    ...
    try {
        const ExactL1Index non_finite_index(
            non_finite_representatives);
    ...
    try {
        const ExactL1Index empty_index(empty_representatives);
