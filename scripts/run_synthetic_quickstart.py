...
DEFAULT_GENERATOR = PROJECT_ROOT / "scripts" / "generate_synthetic_dataset.py"
DEFAULT_BENCHMARK = (
    PROJECT_ROOT / "build" / "nearest_representative_benchmark"
)
DEFAULT_SETUP = PROJECT_ROOT / "experiments" / "synthetic_quickstart_l2.tsv"
DEFAULT_FIGURE_SCRIPT = PROJECT_ROOT / "scripts" / "create_benchmark_figures.py"
...
def setup_paths(path: Path) -> tuple[Path, Path]:
    ...
    with path.open(encoding="utf-8") as source:
        for line_number, raw_line in enumerate(source, start=1):
            ...
            if fields[0] not in ("dataset", "output"):
                continue
